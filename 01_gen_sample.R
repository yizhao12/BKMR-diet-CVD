...
descriptive<-c(FALSE) #whether to run the descriptive analysis on the entire dataset
export<-c(FALSE)
run<-data.frame(descriptive, export)
...
ffq_y0<-plyr::rename(ffq_y0,c("y0_ID"="ID"))
ffq_y3<-plyr::rename(ffq_y3,c("y3_ID"="ID"))
...
for (i in 2:18) { # 1 is ID
  ...
  summary((ffq$y0_FISH+ffq$y3_FISH)/2-ffq$avg_FISH)
  summary((ffq$y0_SOY+ffq$y3_SOY)/2-ffq$avg_SOY)
...
nutr_y0<-plyr::rename(nutr_y0,c("y0_ID"="ID"))
nutr_y3<-plyr::rename(nutr_y3,c("y3_ID"="ID"))
...
for (i in 8:146) {
  ...
  #check the calculation is correct
  summary((nutrients$y0_F60ADSGR+nutrients$y3_F60ADSGR)/2-nutrients$avg_F60ADSGR)
...
out_y0<-plyr::rename(out_y0,c("y0_ID"="ID"))
out_y3<-plyr::rename(out_y3,c("y3_ID"="ID"))
...
for (i in 7:23) {
  ...
  #check the calculation is correct
  summary((outcome$y0_SYST+outcome$y3_SYST)/2-outcome$avg_SYST)

#transform avegage BMIX
...
corrplot(cor(combine_nomiss[grep("avg", names(ffq), value=TRUE)]), type = "upper", order = "hclust", tl.col = "black", tl.srt = 45, main="corr between exposures in average")
...
#Average
ggplot(data = melt(combine_nomiss[c("avg_SYST")]), mapping = aes(x = value)) +
  geom_histogram(bins = 20) + facet_wrap(~variable, scales = 'free_x') +
  labs(title="Distribution of Outcome in average",x="Systolic BP")
...
ggplot(data = melt(combine_nomiss[c("AGE","avg_BMIX_ln","avg_BMIX","INCOME", "EDUC")]), mapping = aes(x = value)) +
  geom_histogram(bins = 20) + facet_wrap(~variable, scales = 'free_x') +
...
#age
plot(combine_nomiss$avg_SYST~combine_nomiss$AGE,main="SBP~Age", col="grey")
abline(lm(avg_SYST~AGE,data=combine_nomiss),col="pink")
lines(lowess(combine_nomiss$AGE,combine_nomiss$avg_SYST),col="green")
legend(67,210,legend=c("regression","lowess"),col=c("pink","green"),lty = c(1,1))
#BMI
plot(combine_nomiss$avg_SYST~combine_nomiss$avg_BMIX,main="SBP~BMI", col="grey")
abline(lm(avg_SYST~avg_BMIX,data=combine_nomiss),col="pink")
lines(lowess(combine_nomiss$avg_BMIX,combine_nomiss$avg_SYST),col="green")
legend(40,210,legend=c("regression","lowess"),col=c("pink","green"),lty = c(1,1))
#ln_BMI
plot(combine_nomiss$avg_SYST~combine_nomiss$avg_BMIX_ln,main="SBP~ln_BMI", col="grey")
abline(lm(avg_SYST~avg_BMIX_ln,data=combine_nomiss),col="pink")
lines(lowess(combine_nomiss$avg_BMIX_ln,combine_nomiss$avg_SYST),col="green")
legend(40,210,legend=c("regression","lowess"),col=c("pink","green"),lty = c(1,1))

#income, educ(categorical)
plot(combine_nomiss$avg_SYST~combine_nomiss$f.INCOME,main="SBP~INCOME", col="pink")
plot(combine_nomiss$avg_SYST~combine_nomiss$f.EDUC,main="SBP~Educ", col="pink")
#income,educ (treating categories as continuous)
plot(combine_nomiss$avg_SYST~combine_nomiss$INCOME,main="SBP~INCOME(cat)", col="grey")
abline(lm(avg_SYST~INCOME,data=combine_nomiss),col="pink")
lines(lowess(combine_nomiss$INCOME,combine_nomiss$avg_SYST),col="green")
legend(5.5,210,legend=c("regression","lowess"),col=c("pink","green"),lty = c(1,1))

plot(combine_nomiss$avg_SYST~combine_nomiss$EDUC,main="SBP~Educ(cat)", col="grey")
abline(lm(avg_SYST~EDUC,data=combine_nomiss),col="pink")
lines(lowess(combine_nomiss$EDUC,combine_nomiss$avg_SYST),col="green")
legend(5.5,210,legend=c("regression","lowess"),col=c("pink","green"),lty = c(1,1))
}

# Final cleaning ----------------------------------------------------------
#only keep average intakes
combine_nomiss_avg<-select(combine_nomiss, -starts_with(c("y0_")))
combine_nomiss_avg<-select(combine_nomiss_avg, -starts_with(c("y3_")))

#rename average variables
names(combine_nomiss_avg)<-sub("avg_","",names(combine_nomiss_avg))
...
summary(lm(SYST~FRUITS+VEGTABLS+FISH+REDMEAT+POULTRY+SOY+NUTS+GRAINS+WHLGRNS+MILKS+DAIRY+WINE+BEER+LIQUOR+POP+COFFEE
           +AGE+BMIX_ln+INCOME+EDUC,data=combine_nomiss_avg))
# 
# 
# 
# Prepare for random sample dataset ---------------------------------------
...
## Export
if (export==TRUE) {
write.csv(combine_nomiss_avg,"/Users/Yi/Box Sync/BKNR/WHIOS/WHIOS_test/dataset/v1_clean.csv")
}
