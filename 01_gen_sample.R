...
ggplot(data = melt(combine_nomiss[c("AGE","BMIX","INCOME", "EDUC")]), mapping = aes(x = value)) + 
  geom_histogram(bins = 20) + facet_wrap(~variable, scales = 'free_x') + 
...
set.seed(123)
sample<-combine_nomiss[sample(1:nrow(combine_nomiss),1500, replace=FALSE), ]
...
#Export
write.csv(sample,"/Users/Yi/Box Sync/BKNR/WHIOS/WHIOS_test/sample_1500.csv")
